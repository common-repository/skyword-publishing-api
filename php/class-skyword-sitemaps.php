...
class Skyword_API_Sitemaps {
	...
	public function skyword_sitemaps_generator() {
		...
		if  ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'skyword-sitemap.xml') !== false) {
			if ( $options['skyword_api_generate_all_sitemaps'] ) {
		...
		} else if ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'skyword-pages-sitemap.xml') !== false) {
			if ( $options['skyword_api_generate_pages_sitemaps'] ) {
				header( 'Content-type: text/xml' );
				// Below function returns appropriately sanitized XML
				// phpcs:ignore
		...
		} else if ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'skyword-categories-sitemap.xml') !== false) {
			if ( $options['skyword_api_generate_categories_sitemaps'] ) {
				header( 'Content-type: text/xml' );
				// Below function returns appropriately sanitized XML
				// phpcs:ignore
		...
		} else if ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'skyword-tags-sitemap.xml') !== false) {
			if ( $options['skyword_api_generate_tags_sitemaps'] ) {
				header( 'Content-type: text/xml' );
				// Below function returns appropriately sanitized XML
				// phpcs:ignore
		...
		} else if ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'skyword-google-news-sitemap.xml') !== false) {
			if ( $options['skyword_api_sgenerate_news_sitemaps'] ) {
				header( 'Content-type: text/xml' );
				// Below function returns appropriately sanitized XML
				// phpcs:ignore
			...
			die;
		} else if ( strpos(esc_attr(empty($_SERVER['REQUEST_URI'])), 'robots.txt') !== false) {
			...
			if ( $options['skyword_api_generate_all_sitemaps'] ==1) {
				print "Sitemap: " . esc_url(get_site_url()) . "/skyword-sitemap.xml \n";
			}
			if ( $options['skyword_api_generate_pages_sitemaps'] == 1 ) {
				print "Sitemap: " . esc_url(get_site_url()) . "/skyword-pages-sitemap.xml \n";
			}
			if ( $options['skyword_api_generate_categories_sitemaps'] == 1) {
				print "Sitemap: " . esc_url(get_site_url()) . "/skyword-categories-sitemap.xml \n";
			}
			if ( $options['skyword_api_generate_tags_sitemaps'] == 1) {
				print "Sitemap: " . esc_url(get_site_url()) . "/skyword-tags-sitemap.xml \n";
			}
			if ( $options['skyword_api_generate_news_sitemaps'] ==1 ) {
	...
	private function skyword_generate_tags_sitemaps() {
		$rows      = get_tags();
		$xmlOutput = '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.5">';
	...
	private function skyword_generate_google_news_sitemaps() {
		$args = array(
    		...
    		// This query triggers a slow query warning, but is necessary for the functionality
			// phpcs:ignore
			'meta_query'       => array(
				array(
...
$skyword_sitemaps = new Skyword_API_Sitemaps;